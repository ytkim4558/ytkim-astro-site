...
이번 사건의 핵심은 redirect가 아니라 **source repo를 잘못 잠근 사고를 복구한 것**이다. GitHub Pages 루트 URL을 끄려다 같은 GitHub repo가 여전히 Astro source로 쓰이고 있다는 사실을 놓쳤고, 그 결과 push가 막혔다.
...
- 로컬 source: `C:\Users\ytkim\projects\ytkim-astro-site`
- GitHub remote: 당시에는 예전 username Pages repo 이름을 아직 사용
...
그래서 repo 이름을 바꾸면 user site URL이 끊긴다는 판단으로 remote repo 이름을 변경했다. 이 판단 자체는 맞았지만, 중간에 repo를 `archived=true`로 잠그면서 문제가 생겼다.
...
- `astro.config.mjs`: `site`를 Cloudflare Pages URL로 변경
- `public/robots.txt`: sitemap URL을 Cloudflare Pages로 변경
- `public/sitemap.xml`: 모든 URL을 Cloudflare Pages로 변경
...
- GitHub repo homepage: Cloudflare Pages URL
...
1. source repo와 legacy URL repo가 같은 GitHub repo인지 확인한다.
2. canonical live URL을 정한다.
...
이 글의 교훈은 URL 변경보다 **source repo를 archive로 잠그지 말 것**이다.
