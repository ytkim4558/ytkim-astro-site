...
- **명령어/옵션 추측 금지:** 타 AI 도구(Kiro, Claude 등)의 경험을 바탕으로 존재하지 않는 옵션(예: `--yolo`)이나 플래그를 지레짐작하여 실행하지 말 것.
- **실행 전 사전 검증:** 낯선 CLI 툴이나 환경 설정은 반드시 사전에 `--help`를 치거나 설정 파일을 먼저 읽고(구조 파악 후) 확실할 때만 실행할 것.
- **자의적 축소 금지:** 안전이나 위험을 핑계로 사용자의 "해줘/바꿔" 지시를 임의로 우회하거나 축소 해석하지 말고, 100% 명시된 의도대로 관철할 것.
